...
namespace PROMPERU.PERUMODA.DA
{
    public static class UsuarioDA
    {
        private static readonly string cadenaConexion =
            ConfigurationManager.ConnectionStrings["BDPeruModa"].ConnectionString;
        ...
        public static void InsertarUsuario(UsuarioBE usuario)
        {
            ...
            try
            {
                ...
                command.Parameters.Add("@Usua_CorreoElectronico", SqlDbType.VarChar).Value =
                    usuario.UsuarioCorreoElectronico;
                command.Parameters.Add("@Usua_TipoDocumento", SqlDbType.VarChar).Value = usuario.UsuarioTipoDocumento;
                command.Parameters.Add("@Usua_NumeroDocumento", SqlDbType.VarChar).Value =
                    usuario.UsuarioNumeroDocumento;
                ...
                command.Parameters.Add("@Usua_AutorizoCompartirDatos", SqlDbType.Bit).Value =
                    usuario.UsuarioAutorizoCompartirDatos;
        ...
        public static UsuarioBE ListarUsuario(UsuarioBE usuario)
        {
            ...
            using (SqlConnection conexion = new SqlConnection(cadenaConexion))
            {
                ...
                {
                    // Pasamos los parámetros
                    command.Parameters.Add("@Usua_CorreoElectornico", SqlDbType.VarChar).Value =
                        usuario.UsuarioCorreoElectronico;
                    command.Parameters.Add("@Usua_NumeroDocumento", SqlDbType.VarChar).Value =
                        usuario.UsuarioNumeroDocumento;
        ...
        public static List<UsuarioBloqueBE> ListarUsuarioBloque()
        {
            List<UsuarioBloqueBE> usuarioBloques = new List<UsuarioBloqueBE>();

            // Conexión a la BD
            using (SqlConnection conexion = new SqlConnection(cadenaConexion))
            {
                // Creamos un objeto command
                SqlCommand command = new SqlCommand("USP_Usuario_LIS", conexion)
                {
                    // Definimos que vamos a usar un procedimiento almacenado.
                    CommandType = CommandType.StoredProcedure
                };
                ...
                try
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            UsuarioBloqueBE usuarioBloque = new UsuarioBloqueBE
                            {
                                UsuarioId = Convert.ToInt32(reader["UsuarioId"]),
                                UsuarioNombres = reader["UsuarioNombres"] == DBNull.Value
                                    ? string.Empty
                                    : reader["UsuarioNombres"].ToString(),
                                UsuarioApellidos = reader["UsuarioApellidos"] == DBNull.Value
                                    ? string.Empty
                                    : reader["UsuarioApellidos"].ToString(),
                                UsuarioCorreoElectronico = reader["UsuarioCorreoElectronico"] == DBNull.Value
                                    ? string.Empty
                                    : reader["UsuarioCorreoElectronico"].ToString(),
                                UsuarioRazonSocial = reader["UsuarioRazonSocial"] == DBNull.Value
                                    ? string.Empty
                                    : reader["UsuarioRazonSocial"].ToString(),
                                UsuarioTipoDocumento = reader["UsuarioTipoDocumento"] == DBNull.Value
                                    ? string.Empty
                                    : reader["UsuarioTipoDocumento"].ToString(),
                                UsuarioNumeroDocumento = reader["UsuarioNumeroDocumento"] == DBNull.Value
                                    ? string.Empty
                                    : reader["UsuarioNumeroDocumento"].ToString(),
                                UsuarioTipo = reader["UsuarioTipo"] == DBNull.Value
                                    ? string.Empty
                                    : reader["UsuarioTipo"].ToString(),
                                UsuarioAutorizoCompartirDatos = Convert.ToBoolean(reader["UsuarioAutorizoCompartirDatos"]),
                                PaisNombre = reader["PaisNombre"] == DBNull.Value
                                    ? string.Empty
                                    : reader["PaisNombre"].ToString(),
                                UsuarioRegion = reader["UsuarioRegion"] == DBNull.Value
                                    ? string.Empty
                                    : reader["UsuarioRegion"].ToString(),
                                UsuarioTelefono = reader["UsuarioTelefono"] == DBNull.Value
                                    ? string.Empty
                                    : reader["UsuarioTelefono"].ToString(),
                                UsuarioFechaCreacion = reader["UsuarioFechaCreacion"] == DBNull.Value
                                    ? DateTime.Now
                                    : Convert.ToDateTime(reader["UsuarioFechaCreacion"]),
                                BloqueNombre = reader["BloqueNombre"] == DBNull.Value
                                    ? string.Empty
                                    : reader["BloqueNombre"].ToString()
                            };

                            usuarioBloques.Add(usuarioBloque);
                        }
                    }
                }
                catch (Exception)
